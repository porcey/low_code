...
<template>
    <ul>
        ...
        <li v-for="(s, key, index) in schema" :key="index">
            ...
            <!-- <div>value:{{value}}</div> -->
            <!-- <div>{{s}}</div> -->
            <component :key="index" :is="getComponents(s.type)" v-bind="s" :schema="s" v-model="value[key]">
                <custom-schema-template v-if="s.child" :schema="s.child" v-bind="s.child" :value="value[key]">
