<template>
    <div id="root" @click.stop="widgetChosen">
        ...
        <div v-if="isCurComponent(theWidget.id)" class="box-tab">
            <template>
                <i class="copy el-icon-document-copy" @click.stop="copyWidegt(theWidget.id)"></i>
                <i class="del el-icon-delete-solid" @click.stop="delWidegt(theWidget.id)"></i>
            </template>
        </div>
        <!-- 子组件插槽 -->
        <slot></slot>
...
export default {
    ...
    methods: {
        ...
        isCurComponent(id) {
            ...
            if (this.curComponent?.id != id) { this.isChosen = false }
